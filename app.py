...
class TimeSheet(Queryable):
    def get(self, row_id):
        timesheet = {}
        timesheet["row_id"] = row_id
        result = self.executeQueryJson("get", timesheet)
        return result, 200
    
    def put(self):
        arg = parser.parse_args()
        timesheet = json.loads(arg['timesheet'])
        result = self.executeQueryJson("put", timesheet)
        return result, 201
    
class TimeSheets(Queryable):
    def get(self):
        result = self.executeQueryJson("get")
        return result, 200
...
# Create API routes
api.add_resource(TimeSheet, '/timesheet', '/timesheet/<row_id>')
api.add_resource(TimeSheets, '/timesheets')
